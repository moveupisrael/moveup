...
export default function ErrorReporter({ error, reset }: ReporterProps) {
  ...
  useEffect(() => {
    ...
    const onError = (e: ErrorEvent) => {
      // ✅ התעלם משגיאות רשת רגילות (fetch failures)
      if (e.message.includes("Failed to fetch") || e.message.includes("NetworkError")) {
        console.log("Network error ignored:", e.message);
        return;
      }
    ...
    const onReject = (e: PromiseRejectionEvent) => {
      // ✅ התעלם משגיאות fetch שמטופלות
      const reason = e.reason?.message ?? String(e.reason);
      if (reason.includes("Failed to fetch") || reason.includes("NetworkError")) {
        console.log("Promise rejection ignored:", reason);
        return;
      }
